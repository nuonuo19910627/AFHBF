...
Page({
  ...
  onLoad: function (option) {
    //this.initNavHeight();
    var addFavorites = wx.getStorageSync('addFavorites') || [];
    if (addFavorites.indexOf(option.F_ID) != -1){
      this.setData({
        collected: true,
      });
    }
    var that = this;
    that.setData({
      productId: option.F_ID,
    });
    console.log("detailgood load");
  ...
  loadProductDetail: function () {
    ...
    qcloud.request({
      ...
      success(res) {
       ...
       // WxParse.wxParse('content', 'html', res.data.data[0], that, 3);
        var imageArr = [];
        imageArr.push(res.data.data[0].F_IMAGE);
        that.setData({
  ...
  addFavorites: function (e) {
    ...
    if (addFavorites == ""){
      ...
      if (addFavorites.indexOf(addid) == -1){
        addFavorites = addFavorites + addid + ",";
        this.setData({
          collected: true,
        });
      }else{
        addFavorites = addFavorites.replace(addid+",", "");
        this.setData({
          collected: false,
        });
      }
    ...
    try {
      console.log('1');
      wx.setStorageSync('addFavorites', addFavorites)
    } catch (e) {
      console.log(e)
    }
    /*
  ...
  addShopCart1: function (e) {
    ...
    var exist = cartItems.find(function (ele) {
      return ele.id === that.data.itemData.F_ID
    })
    console.log(exist)
    if (exist) {
      //如果存在，则增加该货品的购买数量
      ...
      //如果不存在，传入该货品信息
      cartItems.push({
        id: that.data.itemData.F_ID,
        num: that.data.buynum,
        price: that.data.itemData.F_JE,
        title: that.data.itemData.F_NAME,
        image: that.data.itemData.F_IMAGE,
        selected: true
      })
    }
  ...
  addShopCart: function (e) { //立刻购买
    // 初始化toastStr字符串
    var toastStr = this.data.itemData.F_ID+",";
    var count_id = '1,';
